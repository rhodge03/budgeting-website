...
export default function Header() {
  ...
  return (
    <header className="sticky top-0 z-10 bg-brand">
      {/* Top row: brand + desktop tabs + auth */}
      <div className="px-4 flex items-center justify-between h-12">
        ...
        <div className="flex items-center gap-4">
          {isGuest ? (
            ...
            <>
              <span className="text-sm text-gray-300 truncate max-w-[140px]">{user?.email}</span>
              <button
                onClick={logout}
      ...
      {/* Mobile pill tabs — second row */}
      <div className="flex sm:hidden justify-center px-4 pb-2">
        <nav className="bg-white/10 rounded-full p-0.5 flex">
          {tabs.map((tab) => (
            <NavLink
              key={tab.to}
              to={tab.to}
